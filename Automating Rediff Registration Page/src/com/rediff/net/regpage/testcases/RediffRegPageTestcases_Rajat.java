package com.rediff.net.regpage.testcases;

import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

public class RediffRegPageTestcases_Rajat {

	private static final Logger log = LoggerFactory.getLogger(RediffRegPageTestcases_Rajat.class);
	
	public static void main(String[] args) {

		/***
		 * Step 01 : Launch Application using AppUtils.launchApp(); method
		 * Step 02 : Create Object of AutomatingRediffRegPage_Rajat class
		 * 
		 * Test Cases :
		 * 
		 * Test case 01 : Write test case is all images are Displayed or not
		 * Test case 02 : Check if all src url printed in console or not
		 * 
		 * Step 03 : close App using AppUtils.closeApp(); method
		 * */
	}

}
